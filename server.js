...
console.log('FLW_SECRET_KEY:', process.env.FLW_SECRET_KEY ? '✅ SET' : '❌ NOT SET');
console.log('FLW_PUBLIC_KEY:', process.env.FLW_PUBLIC_KEY ? '✅ SET' : '❌ NOT SET');
console.log('FLW_ENV:', process.env.FLW_ENV || 'NOT SET');
...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'Flutterwave v3 backend running!',
    env: {
      secretKey: process.env.FLW_SECRET_KEY ? '✅ Set' : '❌ Missing',
      publicKey: process.env.FLW_PUBLIC_KEY ? '✅ Set' : '❌ Missing',
      env: process.env.FLW_ENV || 'sandbox'
    }
  });
});
